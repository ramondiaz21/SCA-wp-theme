 ...
 */
   ...
   $sidebar_layout_config = kiamo_get_option('archive_portfolio_sidebar', 'right-sidebar'); 
   $left_sidebar = kiamo_get_option('archive_portfolio_left_sidebar', 'portfolio_sidebar');
   $right_sidebar = kiamo_get_option('archive_portfolio_right_sidebar', 'portfolio_sidebar');
   ...
   $columns_lg = kiamo_get_option('portfolio_columns_lg', '4');
   $columns_md = kiamo_get_option('portfolio_columns_md', '3');
   $columns_sm = kiamo_get_option('portfolio_columns_sm', '2');
   $columns_xs = kiamo_get_option('portfolio_columns_xs', '1');
          ...
          <div id="wp-content" class="wp-content clearfix">
            <div class="lg-block-grid-<?php echo esc_attr($columns_lg) ?> md-block-grid-<?php echo esc_attr($columns_md) ?> sm-block-grid-<?php echo esc_attr($columns_sm) ?> xs-block-grid-<?php echo esc_attr($columns_xs) ?>">
               <?php while ( have_posts() ) : the_post(); ?>
                  <div class="item-columns"><?php get_template_part( 'templates/portfolio/content', 'item-v1' ); ?></div>
               <?php endwhile; ?>  
            </div>
          </div>    
        </div>      
