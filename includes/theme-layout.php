...
================================================== */
if ( ! function_exists( 'kiamo_base_sidebar' ) ) {
     function kiamo_base_sidebar() {
        // VARIABLES
        global $post;

        // DEFAULT SIDEBAR CONFIG
        $default_sidebar_config = kiamo_get_option('default_sidebar_config', ''); 
        $default_left_sidebar   = kiamo_get_option('default_left_sidebar', ''); 
        $default_right_sidebar  = kiamo_get_option('default_right_sidebar', '');
        ...
        if ( is_search() || is_archive() || is_author() || is_category() || is_home() ) {
          $default_sidebar_config = kiamo_get_option('archive_sidebar_config', ''); 
          $default_left_sidebar   = kiamo_get_option('archive_sidebar_left', '');
          $default_right_sidebar  = kiamo_get_option('archive_sidebar_right', ''); 
        }
        ...
        if ( $post && is_singular() ) {
          $sidebar_config = get_post_meta( $post->ID, 'kiamo_sidebar_config', true );
          $left_sidebar   = get_post_meta( $post->ID, 'kiamo_left_sidebar', true );
          $right_sidebar  = get_post_meta( $post->ID, 'kiamo_right_sidebar', true );
        }
...
function kiamo_sidebar_global($sidebar_layout_config, $sidebar_left_config, $sidebar_right_config){
...
function kiamo_base_layout($template){
  $post_id = kiamo_id();
  $sidebar_var           = kiamo_base_sidebar();
  ...
  $sidebar_config = kiamo_sidebar_global($sidebar_var['config'], $sidebar_var['left'], $sidebar_var['right']);
  extract($sidebar_config);
  
  $page_title_style = get_post_meta($post_id, 'kiamo_page_title_style', true );
  $page_full_width = get_post_meta($post_id, 'kiamo_page_full_width', true);
  ...
  <?php do_action( 'kiamo_before_page_content' ); ?>
    <div class="<?php echo ($page_full_width ? 'container-full' : 'container'); ?>">
    <div class="content-page-wrap">
      <?php do_action( 'kiamo_before_main_content' ); ?>
       <div class="main-page-content base-layout row <?php echo esc_attr($page_wrap_class); ?>">
          
            <div class="content-page <?php echo esc_attr($main_content_config['class']); ?>">
              <div class="content-page-inner">   
                  <?php kiamo_get_template($template) ?>
              </div>    
            </div>      
        ...
        <?php do_action( 'kiamo_after_main_content' ); ?>
      </div>
    </div>
...
if ( ! function_exists( 'kiamo_get_template' ) ) {
    function kiamo_get_template( $template ) {
