   ...
   <div class="clearfix widget-content">
      ...
      <div class="tabs-container clearfix">
         <div class="tab-content">
             <?php $i = 0; ?>
             <?php foreach ($line_data as $key => $value): $i++; ?>
                <div class="tab-pane <?php if($i==1) echo 'active'; ?>" id="<?php echo esc_attr($key).'-'.$_id; ?>" data-loaded="false" data-number="<?php echo esc_attr($number) ?>" data-style="<?php echo esc_attr($value['style']) ?>" data-type="<?php echo esc_attr($value['type']) ?>" data-columns="<?php echo esc_attr($columns_count) ?>" data-categories="<?php echo esc_attr($value['category']) ?>" data-row="<?php echo esc_attr($carousel_row) ?>">
                  <div class="shop-products">
                    <div class="tab-content-products">
                      <?php if($i==1){ 
                        $loop = kiamo_woocommerce_query($value['type'], $number, $value['category']);
                        if($loop->have_posts()){
